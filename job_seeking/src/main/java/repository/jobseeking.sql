...
select post.id,post.post_title,post.describe,post.date_submitted,post.img,u.use_name,c.post_category from post
left join `use` u on u.id_use = post.id_use
left join category c on c.id_category = post.id_category
group by post.id
order by post.id;